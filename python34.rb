class Python34 < Formula
  ...
  def install
    ...
    # Demos and Tools
    system "make", "frameworkinstallextras", "PYTHONAPPSDIR=#{share}/python3.4"
    ...
    # Let python3 symlink the pkgconfig files
    ...
    # Let python3 install the man pages
    rm_rf share/man
    %W[idle3 idle3.4 pydoc3 python3 python3-config pyvenv].each do |e|
      rm bin/e
    end
  end
  ...
  def post_install
    ...
    mv bin/"pip", bin/"pip#{xy}"
    mv bin/"easy_install", bin/"easy_install-#{xy}"
    mv bin/"wheel", bin/"wheel#{xy}"

    # post_install happens after link
    %W[pip#{xy} easy_install-#{xy} wheel#{xy} python#{xy}].each do |e|
